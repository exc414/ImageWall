...
public abstract class CustomBtnBase extends View
{
    ...
    private void circleLongAnimation()
    {
        ...
        circleFade.addListener(new AnimListener()
        {
            ...
            protected void animationEnded(Animator animation)
            {
                ...
                //Once circle animation is over fade out the selected state.
                //selectedAnimation(baseAlpha, 0);
            }
        });
    }
    ...
    {
        float clickBoundX = motionEvent.getX();
        float clickBoundY = motionEvent.getY();

        //The click bounds need to match the actual drawn bounds of the button and not all
        //the width/height area.
        if((buttonBounds.top <= clickBoundY && buttonBounds.bottom >= clickBoundY)
                && (buttonBounds.left <= clickBoundX && buttonBounds.right >= clickBoundX))
        {
            //UserPrivateFB first presses
            if(motionEvent.getAction() == MotionEvent.ACTION_DOWN)
            {
                circlePosX = clickBoundX;
                circlePosY = clickBoundY;
            ...
            if(motionEvent.getAction() == MotionEvent.ACTION_UP)
            {
                ...
                //Need to be here and no outside or it will be called twice. For DOWN and UP.
                performClick();
            }
        }
